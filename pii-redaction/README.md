...
This example redacts PII/PCI data from requests, but its main purpose is to show
**how to benchmark a custom policy**. The policy takes a different code path per
request — header masking, plain-text regex scanning, and shallow/deep JSON
masking — so each flow has a distinct performance profile worth measuring.

## Benchmarking

The `pdk-unit` harness runs your policy in an in-process emulated Flex host, with
no Docker, no network, and no real gateway. A request completes in microseconds,
which makes the same harness you use for unit tests a solid foundation for
micro-benchmarks: you can measure the cost of a policy's code paths in isolation.
...
```
cargo bench --bench redaction
```

*For more information about writing unit tests with `pdk-unit`, see [PDK Unit Testing](https://docs.mulesoft.com/pdk/latest/policies-pdk-unit).*
...
### Test
The `make test` goal runs the unit tests in `src/tests/`, which drive the policy
through the in-process `pdk-unit` emulated environment.

*For more information about writing unit tests, see [Writing Unit Tests](https://docs.mulesoft.com/pdk/latest/policies-pdk-unit-tests).*
